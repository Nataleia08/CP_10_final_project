...
def search_phone(text: str):
    phone = []
    s = re.findall(
        r"[+380]?[(]?[0-9]{2}[)]?[0-9]{3}[-]?[0-9]{1,2}[-]?[0-9]{2,3}\b", " ".join(text))
    if s != None:
        for i in s:
            i_phone = Phone()
            i_phone.value = i
            phone.append(i_phone)
            return phone, (" ".join(input_list).replace(i, "")).split(" ")


def search_birthday(text: str) -> str:
    date_birthday = None
    for i in text:
...
# ----------------------------Розпізнавання введенної команди-----------------------
        ...
        if (input_com == "add") or (input_com == "change"):
            command_phone, input_list = search_phone(input_list)
            command_birthday.value = search_birthday(input_list)
        if input_com == "hello":
            ...
            try:
                if (not name) or (len(command_phone) == 0):
                    ...
                    continue
                new_record = Record(
                    command_name, command_phone, command_birthday)
